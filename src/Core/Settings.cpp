...
DECLARE(Bool, query_plan_ie_join_select_conditions_by_selectivity, true, R"(
When the `JOIN ON` expression contains more inequality conditions between the joined tables than the two the IEJoin algorithm (see `ie_join` in [`join_algorithm`](#join_algorithm)) uses as its key conditions, choose the two with the smallest estimated joint selectivity instead of the first two in syntax order. The conditions not chosen as keys are evaluated per pair of rows passing both key conditions, so the least selective choice of keys minimizes that work. Selectivity is estimated from the column min/max statistics (see the `basic` type in [Column statistics](/reference/engines/table-engines/mergetree-family/mergetree#column-statistics)); when the estimate is unavailable for any of the conditions, the first two in syntax order are used, as if this setting were disabled.
)", 0) \
...
- ie_join
 ...
 The position in the list sets the priority: listed after other algorithms, as in the default value, IEJoin is used only when they do not apply (the `ON` section has no equality conditions); listed first, it is used whenever the `ON` section has two inequality conditions. The remaining conditions (including equalities) are applied as a filter over the join result for `ALL INNER JOIN`, and evaluated inside the operator as a residual condition affecting matching for the other kinds. When the `ON` section has more than two eligible inequality conditions, the two used by the algorithm are chosen by their estimated selectivity from column statistics (see [`query_plan_ie_join_select_conditions_by_selectivity`](#query_plan_ie_join_select_conditions_by_selectivity)). Without `ie_join` in the list, an `INNER JOIN` with only inequality conditions is executed as a `CROSS JOIN` with a filter, and the other kinds are not supported.
